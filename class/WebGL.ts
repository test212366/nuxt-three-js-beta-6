	...
	import fragmentShader from '../shaders/fragment.frag';
 	//@ts-ignore
	import vertexShader from '../shaders/vertexParticles.vert'
	...
	// import {GlitchPass} from 'three/examples/jsm/postprocessing/GlitchPass'
 	...
 	export class WebGLScene {
		...
		constructor(options: any) {
		  ...
		  this.camera.position.set(0, 0, 730) 
		...
		getData() {
			...
			this.svg.forEach((path:any, j:number) => {
	...
	addObjects() {
		...
		let mapMesh = new THREE.Mesh(
			new THREE.PlaneGeometry(2048, 1024, 1, 1),
			new THREE.MeshBasicMaterial({
				color: this.width <= 1000 ? 0x282828 : 0x1A1A1A ,
				map:  texture
			})
		) 
